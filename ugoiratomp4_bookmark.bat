...
for %%f in (*.zip) do (
    if not exist "..\..\..\ugoira\%%~nf\" (
        ...
        magick convert -layers optimize -delay 66.7 -loop 0 *.png "%%~nf_15f.gif"
        magick convert -layers optimize -delay 100 -loop 0 *.png "%%~nf_10f.gif"
        magick convert -layers optimize -delay 200 -loop 0 *.png "%%~nf_5f.gif"
